...
import { FiChevronDown, FiChevronUp, FiPlayCircle, FiFileText, FiImage, FiVideo, FiCheckCircle, FiArrowLeft, FiDownload, FiChevronLeft, FiChevronRight, FiClock } from 'react-icons/fi';
...
const Learn = () => {
    ...
    api.get(`/batches/${batchId}`)
      .then(r => {
        ...
        if (batchData.curriculum?.length > 0) {
          setExpandedModules({ [batchData.curriculum[0]._id || 0]: true });
  ...
  const curriculumItems = batch?.curriculum?.flatMap(m => m.items) || [];
  const currentIndex = curriculumItems.findIndex(i => i === selectedItem);
  const prevItem = currentIndex > 0 ? curriculumItems[currentIndex - 1] : null;
  const nextItem = currentIndex < curriculumItems.length - 1 ? curriculumItems[currentIndex + 1] : null;

  const handlePrevItem = () => { if (prevItem) setSelectedItem(prevItem); };
  const handleNextItem = () => { if (nextItem) setSelectedItem(nextItem); };

  const renderContent = () => {
    if (!selectedItem) return <div className="empty-content">Select an item to start learning</div>;
    ...
    switch (selectedItem.type) {
          ...
          <div className="content-viewer">
            ...
            <div className="video-wrapper">
              {ytId ? (
                ...
                <div className="error-placeholder">Invalid Video URL</div>
              )}
            </div>
          </div>
        );
      case 'text':
        return (
          <div className="content-viewer text-content">
            <h2>{selectedItem.title}</h2>
          ...
          <div className="hero-info">
            ...
            <div className="hero-footer">
               ...
               <div className="progress-stats">
                  ...
                  <div className="progress-bar-container" style={{width: 150}}><div className="progress-bar-fill" style={{width: '20%'}}></div></div>
               </div>
            </div>
          </div>
        </div>
          ...
          <div className="player-section">
            <div className="card player-card ">
              <div className="player-nav">
                <button className="btn btn-secondary btn-sm" onClick={handlePrevItem} disabled={!prevItem}><FiChevronLeft /> Previous</button>
                <span className="item-count">Content {currentIndex + 1} of {curriculumItems.length}</span>
                <button className="btn btn-secondary btn-sm" onClick={handleNextItem} disabled={!nextItem}>Next <FiChevronRight /></button>
              </div>
            ...
            <button className="btn btn-secondary mt-8" onClick={() => navigate(`/dashboard/${batchId}`)}><FiArrowLeft /> Back to Dashboard</button>
          </div>

          <aside className="curriculum-sidebar-v2">
            <div className="card sidebar-card">
              <div className="sidebar-header">
                <h3><FiFileText /> Curriculum</h3>
              </div>
              <div className="sidebar-list">
                {batch.curriculum?.map((module, mIdx) => (
                  <div key={mIdx} className="module-section">
                    <button 
                      className={`module-toggle ${expandedModules[module._id || mIdx] ? 'active' : ''}`}
                      onClick={() => toggleModule(module._id || mIdx)}
                    >
                      <div className="module-title-group">
                        <span className="module-label">MODULE {mIdx + 1}</span>
                        <h4>{module.title}</h4>
                      </div>
                      {expandedModules[module._id || mIdx] ? <FiChevronUp /> : <FiChevronDown />}
                    </button>
                    
                    {expandedModules[module._id || mIdx] && (
